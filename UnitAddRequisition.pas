...
begin
  ...
  Try
    ...
    Close;
    Application.MessageBox('Заявка успешно зарегистрирована',
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select * from MASS_19.КП_Клиенты_ИВЦ where "Работает"='+ QuotedStr('y'));
    open;
  end;
...
begin
  ...
  begin
    Close;
    SQL.Clear;
    SQL.Add('select "Тип работы", "Время решения" from MASS_19.КП_КЛИЕНТЫ_ИВЦ join MASS_19.КП_КАТАЛОГ_УСЛУГ ККУ on КП_КЛИЕНТЫ_ИВЦ."Категория значимости" = ККУ."Категория значимости"'
...
begin
  ...
  if Form1.ADOQueryRoleDispatcher.IsEmpty then
    DBEditClient.Text := Form1.ADOQueryUser.FieldByName('ЭЛЕКТРОННАЯ_ПОЧТА').AsString;
